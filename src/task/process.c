...
// All of the processes running on the system
...
static int process_map_elf(struct process* process)
{
    ...
    for (int i = 0; i < header->e_phnum; i++)
    {
        ...
        res = paging_map_to(process->task->page_directory, paging_align_to_lower_page((void*)phdr->p_vaddr), paging_align_to_lower_page(phdr_phys_address), paging_align_address((void*)((int)phdr_phys_address + phdr->p_memsz)),flags);
        if(ISERR(res))
        {
            break;
        }
