...
void fat16_get_full_relative_filename(struct fat_directory_item* item, char* out, int max_len)
{
    ...
    char* out_tmp = out;
    fat16_to_proper_string(&out_tmp, (char*) item->filename);
    if(item->ext[0] != 0x00 && item->ext[0] != 0x20)
    {
        *out_tmp++ = '.';
        fat16_to_proper_string(&out_tmp, (char*) item->ext);
    }
}
...
{
    struct fat_item* f_item = kzalloc(sizeof(struct fat_directory_item));
    if(!f_item)
    {
        return 0;
    }
